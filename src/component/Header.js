...
const Header = () => {
    ...
    const handleSearch = (e) => {
        const searchValue = e.target.value;
        const tam = post.filter((subpost) => subpost.TieuDe.toLowerCase().includes(searchValue.toLowerCase()));
        if (searchValue !== "") {
            setPostFilter(tam)
        } else {
            setPostFilter([])
    ...
    return loading ? <Loading /> : (
        <div className="container">
            <div className="row height-100 mau1">
                ...
                <div className="col-6" >
                    <div className="search-box">
                        <input type="text" className="search-text" placeholder="Tìm kiếm..." onChange={(e) => handleSearch(e)} />
                        <button type="submit" className="search-btn">
                            <i className="fas fa-search" />
                        </button>
                    </div>
                    {
                        ...
                        (
                            <div className="search-result">
                                <ul>
                                    {
                                        postFilter.slice(0, 8).map((subpost) => (
                                            <li key={subpost.id}>
                                                <Link to={chuyenDoiURL(subpost.TieuDe) + "." + subpost.id + ".html"} >{subpost.TieuDe}</Link>
